...
class OAuthClient():
    ...
    def get_access_token(self,auth_token,auth_verifier):
        ...
        response=self.make_request(self.access_url,token=auth_token,additional_params={'oauth_verifier':auth_verifier})
    ...
    def save_user_info_into_db(self,username,password,token,secret):
        ...
        res=AuthTokenModel.all().filter('service =',service).filter('username =',username.lower())
        if res.count()>0:
            db.delete(res)
...
class TwitterClient(OAuthClient):
    ...
    def get_authorization_url(self):
        ...
        return 'https://api.twitter.com/oauth/authorize?oauth_token={0}'.format(token)
